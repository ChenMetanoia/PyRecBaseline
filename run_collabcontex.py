...
def initialize_model_and_trainer(config_dict):
    config_dict[
        "checkpoint_dir"
    ] = f"results/{config_dict['PLM']}/{config_dict['dataset']}/CollabFusion/"

    config_file_list = ["properties/train.yaml", "properties/data.yaml"]
    config = Config(
        model=CollabContex, config_dict=config_dict, config_file_list=config_file_list
    )
...
def run_evaluation_phases(
    trainer, config, logger, train_data, valid_data, test_data, cold_data
):
    # Item Tutoring Phase
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="Electronics",
        type=str,
        help="[Electronics, Office_Products, Grocery_and_Gourmet_Food]",
    )
    parser.add_argument("--num_layers", default=2, type=int, help="number of layers")
    parser.add_argument(
        "--PLM",
        default="instructor-xl",
        help="[all-mpnet-base-v2, all-MiniLM-L6-v2, bert-base-uncased, bge-base-en-v1.5, instructor-xl]",
    )
    ...
    (
        ...
        test_data,
        cold_data,
    ) = initialize_model_and_trainer(config_dict)

    run_evaluation_phases(
        trainer, config, logger, train_data, valid_data, test_data, cold_data
    )
