...
class GeneralGraphDataset(RecBoleDataset):
    ...
    def get_norm_adj_mat(self, direct_graph=False):
        ...
        if direct_graph: # if the graph is not bidirectional, we don't need to add the reverse direction
            edge_index = torch.stack([col, row])
        else: # if the graph is bidirectional, we need to add the reverse direction
    ...
    def get_norm_adj_rating_mat(self):
        r"""Get the normalized interaction matrix of users and items, 
        then multiply it with the rating matrix. The rating matrix is 
        normalized into 0-2.
        .. math::
            A_{hat} = D^{-0.5} \times A \times D^{-0.5}
            x = \frac{rating-0.5}{2}
            f(x) = 2 \times sigmoid(x)
        Returns:
            The normalized interaction matrix in Tensor.
        """
        row = self.inter_feat[self.uid_field]
        ...
        rating_weight = self.inter_feat['rating']
        rating_weight = (rating_weight - 0.5) / 2
        rating_weight = torch.sigmoid(rating_weight)*2
        edge_weight = edge_weight * torch.cat([rating_weight, rating_weight])
